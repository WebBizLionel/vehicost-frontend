...
const slideData = [
    {bgcolor:gColor.lightColor, title:'Bienvenue sur Vehicost', text:'Votre copilote pour suivre et gérer facilement  toutes vos dépenses.', img:<Image1 style={{marginBottom:24}}width={'100%'} height={300}/>},
    {bgcolor:gColor.lightAdditionalColor,text:'Conservez vos justificatifs en toute confiance !'},
...
const WelcomeSlider = () => (
        ...
        renderItem={({ item }) => (
          <View style={[styles.child, { backgroundColor: item.bgcolor }]}>
            <View style={styles.childWrapper}>
                {item.img && item.img}
                {item.text && <Text style={{...styles.text, ...global.defaultH6}}>{item.text}</Text> }
            </View>
          </View>
        )}
      />
  ...
  const styles = StyleSheet.create({
    ...
    text: {lineHeight:27,textAlign: 'center', fontWeight:700, fontStyle:'italic'},
